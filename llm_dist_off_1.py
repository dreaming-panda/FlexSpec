...
class Distributed_Offload_KV_Cache:
...
class DistributedLlama:
    def __init__(self, 
        ...
        # if kv_offload:
        self.kv_cache = Distributed_Offload_KV_Cache(self.config, device="cpu", dtype=dtype)
        self.kv_buffer = KVCacheBuffer(self.config, device=self.device, dtype=dtype)
        # else:
        #     self.kv_cache = Distributed_Offload_KV_Cache(self.config, device=self.device, dtype=dtype)
    ...
    def layer_compute(self, 
        ...
        kv_seq_len += self.kv_buffer.kv_offset
        ...
        ## offloading
        key_states, value_states = self.kv_buffer.update_kv_cache(key_states, value_states, layer_idx)
...
class LLMEngine:
    def __init__(self, 
        ...
        # self.llm = DistributedLlama(model_name, dtype, on_chip_layers=4)
      # local_rank, world_size = distributed_init()
      # device  = torch.device("cuda", local_rank)

      self.llm = DistributedLlama(model_name_or_path=model_name, on_chip_layers=4, local_rank=local_rank, world_size=world_size)
      for rank in range(world_size):
          if local_rank == rank:
              hf_model = LlamaForCausalLM.from_pretrained(model_name, torch_dtype=torch.float16)
              self.llm.init_parameters(hf_model=hf_model)
              del hf_model
          dist.barrier()   
      self.callables = {}
      # self.mempool = None
    ...
    def inference(self,
            ...
            if dec_length in self.callables.keys():
                logits = self.callables[dec_length](input_ids, position_ids, attention_mask[:-1, : -1][None, None, :, :])
            else:
                logits = self.llm.inference(input_ids, position_ids, attention_mask[:-1, : -1][None, None, :, :])
            return logits
