...
class KVCacheBuffer:
    ...
    def copy_kv(self,
        ...
        # print(self.k_cache, k_cache)
        # k_cache = k_cache.to(self.k_cache.device)
        # v_cache = v_cache.to(self.v_cache.device)
        # print(self.k_cache.device, k_cache.device)
        self.k_cache[...,:kv_len,:].copy_(k_cache[...,:kv_len,:].to(self.k_cache.device), non_blocking=True)
        self.v_cache[...,:kv_len,:].copy_(v_cache[...,:kv_len,:].to(self.v_cache.device), non_blocking=True)
        self.kv_offset = kv_len
        ## check devices
        # self.k_cache.index_copy_(dim=-2, index=storage_ids, source=k_cache)
        # self.v_cache.index_copy_(dim=-2, index=storage_ids, source=v_cache)
    ...
    #     self.k_cache.index_copy_(dim=-2, index=storage_ids, source=new_k_cache)
    #     self.v_cache.index_copy_(dim=-2, index=storage_ids, source=new_v_cache)

    #     ## added this offloading
...
class KV_Cache:
    ...
    #         self.k_cache = self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].to(kv_buffer.k_cache.device)
    #         self.v_cache = self.v_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].to(kv_buffer.v_cache.device)
    ...
    #         temp1 = kv_buffer.k_cache[..., self.kv_offset: kv_buffer.kv_offset, :]
    #         temp2 = kv_buffer.v_cache[..., self.kv_offset: kv_buffer.kv_offset, :]
    #         print( self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].shape,kv_buffer.k_cache[..., self.kv_offset: kv_buffer.kv_offset, :].shape)
    #         self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].copy_(temp1.squeeze(dim=0), non_blocking=True)
    #         self.v_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].copy_(temp2.squeeze(dim=0), non_blocking=True)
    #         print( self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].shape,kv_buffer.k_cache[..., self.kv_offset: kv_buffer.kv_offset, :].shape)
    #         # self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :] = self.k_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].unsqueeze(dim=0)
    #         # self.v_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :] = self.v_cache[layer_idx][..., self.kv_offset: kv_buffer.kv_offset, :].unsqueeze(dim=0)
    #         if layer_idx == self.num_layers - 1:
...
class LLMLayer:
    ...
    def init_parameters(self, hf_layer: LlamaDecoderLayer):
        ...
        self.wq :torch.Tensor= self.wq.split((self.num_heads * self.head_dim) // self.world_size, dim=0)[self.rank].pin_memory()
...
class LLM:
    ...
    def pre_attention_compute(
        ...
        # print(hidden_states.device, wq.device, wk.device, wv.device)
        # wq = wq.to(hidden_states.device)
        # wk = wk.to(hidden_states.device)
        # wv = wv.to(hidden_states.device)
    ...
    def post_attention_compute(
        ...
        ### Start of TP_MLP()
        # up_proj = up_proj.to(hidden_states.device)
        # down_proj = down_proj.to(hidden_states.device)
        # gate_proj = gate_proj.to(hidden_states.device)
